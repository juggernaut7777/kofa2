        ...
        <div className={`min - h - screen font - ['SF_Pro_Display', -apple - system, sans - serif] ${isDark ? 'bg-[#0a0a14]' : 'bg-[#fafaff]'} `}>
            ...
            <div className="relative max-w-md mx-auto pb-28">
                ...
                <header className={`sticky top - 0 z - 30 px - 6 pt - 5 pb - 4 ${isDark ? 'bg-[#0a0a14]/70' : 'bg-[#fafaff]/70'} backdrop - blur - 2xl`}>
                    ...
                    <p className={`text - sm ${isDark ? 'text-white/40' : 'text-black/40'} `}>{activeTab === 'orders' ? `${orders.length} total orders` : `${invoices.length} invoices`}</p>
                </header>
                ...
                <div className="flex gap-2 px-6 pt-2">
                    {tabs.map(tab => (
                        <button
                            ...
                            className={`flex items - center gap - 2 px - 4 h - 10 rounded - xl text - sm font - medium transition - all hover: scale - 105 ${activeTab === tab.id ? 'text-white' : isDark ? 'bg-white/[0.03] text-white/40 border border-white/[0.06]' : 'bg-white text-black/40 border border-black/[0.04]'
                                } `}
                        ...
                        <div className="flex gap-2 px-6 pt-4 overflow-x-auto no-scrollbar">
                            {statusFilters.map(f => (
                                <button
                                    ...
                                    className={`px - 4 h - 9 rounded - xl whitespace - nowrap text - sm font - medium transition - all hover: scale - 105 ${activeStatus === f.id ? 'text-white' : isDark ? 'bg-white/[0.03] text-white/40 border border-white/[0.06]' : 'bg-white text-black/40 border border-black/[0.04]'
                                        } `}
                    ...
                    <div className="px-6 pt-5 space-y-3">
                        {invoices.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20">
                                ...
                                <p className={`text - sm text - center ${isDark ? 'text-white/40' : 'text-black/40'} `}>Generate invoices from orders</p>
                            </div>
                        ) : (
                                ...
                                <div key={invoice.invoice_id} className={`p - 4 rounded - 2xl ${isDark ? 'bg-white/[0.03] border border-white/[0.06]' : 'bg-white border border-black/[0.04]'} `}>
                                    <div className="flex items-center justify-between mb-3">
                                        <div>
                                            ...
                                            <p className={`text - xs ${isDark ? 'text-white/40' : 'text-black/40'} `}>{invoice.invoice_id} • {invoice.order_id}</p>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className={`text - xs ${isDark ? 'text-white/30' : 'text-black/30'} `}>{formatDate(invoice.created_at)}</span>
                                        <div className="flex items-center gap-2">
                    ...
                    <div className={`relative w - full max - w - md rounded - t - 3xl overflow - hidden ${isDark ? 'bg-[#0a0a14]' : 'bg-white'} `}>
                        ...
                        <div className="p-6 max-h-[80vh] overflow-y-auto">
                                ...
                                {(showOrderDetail.items || []).map((item, i) => (
                                    <div key={i} className="flex items-center justify-between py-2">
                                        <div>
                                            ...
                                            <p className={`text - xs ${isDark ? 'text-white/40' : 'text-black/40'} `}>x{item.quantity}</p>
                                        </div>
                            ...
                            <div className="space-y-3">
                                <p className={`text - xs font - semibold uppercase tracking - wide ${isDark ? 'text-white/30' : 'text-black/30'} `}>Actions</p>
                                ...
                                {/* Update Status */}
                                <p className={`text - xs font - semibold uppercase tracking - wide pt - 2 ${isDark ? 'text-white/30' : 'text-black/30'} `}>Update Status</p>
